...
import { from, fromPromise } from "../constructors/from";
...
export function flatMap<T, U> ( iterables : AsyncIterableLike<T>, mapper : ( item : T, index : number ) => Promise<AsyncIterableLike<U>> | AsyncIterableLike<U> ) : AsyncIterableIterator<U> {
    return flatten( map<T, AsyncIterableLike<U>>( iterables, mapper ) );
}

export function flatMapLast<T, U> ( iterables : AsyncIterableLike<T>, mapper : ( item : T, index : number ) => Promise<AsyncIterableLike<U>> | AsyncIterableLike<U>, concurrency : number ) : AsyncIterableIterator<U> {
    return flattenLast( map<T, AsyncIterableLike<U>>( iterables, mapper ), concurrency );
}

export function flatMapConcurrent<T, U> ( iterables : AsyncIterableLike<T>, mapper : ( item : T, index : number ) => Promise<AsyncIterableLike<U>> | AsyncIterableLike<U>, concurrency : number ) : AsyncIterableIterator<U> {
    return flattenConcurrent( map<T, AsyncIterableLike<U>>( iterables, mapper ), concurrency );
}

export function flatten<T> ( iterables : AsyncIterableLike<AsyncIterableLike<T>> ) : AsyncIterableIterator<T> {
    return flattenConcurrent( iterables, 1 );
}

export function flattenLast<T> ( iterables : AsyncIterableLike<AsyncIterableLike<T>>, concurrency : number ) : AsyncIterableIterator<T> {
    return flattenConcurrent( iterables, concurrency, true );
}

export function flattenConcurrent<T> ( iterables : AsyncIterableLike<AsyncIterableLike<T>>, concurrency : number, switchFast : boolean = false ) : AsyncIterableIterator<T> {
    ...
    forEach( iterables, async source => {
        ...
        const iter = from( source )[ Symbol.asyncIterator ]();
